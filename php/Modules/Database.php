...
final class Database {
    ...
    public static function get() {
        if (!self::$instance) {
            ...
            self::$instance = new self(
                    $credentials['host'], $credentials['user'], $credentials['password'], $credentials['name'], $credentials['prefix']
            );
        }
    ...
    private function __construct($host, $user, $password, $name = 'lychee', $dbTablePrefix) {
        ...
        if ($connection === false) {
            Response::error(self::connect_error());
        }

        if (self::setCharset($connection) === false) {
            Response::error('Could not set database charset!');
        }

        // Create database
        if (self::createDatabase($connection, $name) === false) {
            Response::error('Could not create database!');
        }

        // Create tables
        if (self::createTables($connection) === false) {
            Response::error('Could not create tables!');
        }

        // Update database
        if (self::update($connection, $name) === false) {
            Response::error('Could not update database and tables!');
        }
    ...
    private static function setCharset($connection) {
        ...
        if ($connection->server_version < 50500) {
            @$connection->set_charset('GBK');
        } else {
    ...
    public static function createDatabase($connection, $name = 'lychee') {
        ...
        if ($connection->select_db($name) === true) {
            return true;
        }
        ...
        if ($connection->select_db($name) === false) {
            return false;
        }
        return true;
    }
    ...
    private static function createTables($connection) {
        ...
        $query = self::prepare($connection, 'SELECT * FROM ?, ?, ?, ?, ?, ? LIMIT 0', array(LYCHEE_TABLE_PHOTOS, LYCHEE_TABLE_ALBUMS, LYCHEE_TABLE_SETTINGS, LYCHEE_TABLE_LOG, LYCHEE_TABLE_TAGS, LYCHEE_TABLE_PHOTOS_TO_TAGS));
        $result = self::execute($connection, $query, null, null);
        if ($result !== false) {
            return true;
        }
        ...
        $exist = self::prepare($connection, 'SELECT * FROM ? LIMIT 0', array(LYCHEE_TABLE_TAGS));
        $result = self::execute($connection, $exist, __METHOD__, __LINE__);

        if ($result === false) {
            // Read file
            $file = __DIR__ . '/../database/tags_table.sql';
            $query = @file_get_contents($file);

            if ($query === false) {
                Log::error($connection, __METHOD__, __LINE__, 'Could not load query for lychee_tags');
                return false;
            }

            // Create table
            $query = self::prepare($connection, $query, array(LYCHEE_TABLE_TAGS));
            $result = self::execute($connection, $query, __METHOD__, __LINE__);
        ...
        // Check if photos to tags table exists
        $exist = self::prepare($connection, 'SELECT * FROM ? LIMIT 0', array(LYCHEE_TABLE_PHOTOS_TO_TAGS));
        $result = self::execute($connection, $exist, __METHOD__, __LINE__);

        if ($result === false) {
            // Read file
            $file = __DIR__ . '/../database/photos_to_tags_table.sql';
            $query = @file_get_contents($file);

            if ($query === false) {
                Log::error($connection, __METHOD__, __LINE__, 'Could not load query for lychee_photos_to_tags');
                return false;
            }

            // Create table
            $query = self::prepare($connection, $query, array(LYCHEE_TABLE_PHOTOS_TO_TAGS, LYCHEE_TABLE_PHOTOS, LYCHEE_TABLE_TAGS));
            $result = self::execute($connection, $query, __METHOD__, __LINE__);
    ...
    public static function insertId($connection, $function, $line) {
        // Check dependencies
        Validator::required(isset($connection), __METHOD__);

        if($connection->insert_id == 0) {
            if($logging === true) {
                Log::notice($connection, $function, $line, "No previous insert ID found on connection.");
            }
            return false;
        }
