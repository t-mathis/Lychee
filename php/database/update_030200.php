...
while ($photoWithTags = $photosWithTags->fetch_object()) {
    ...
    foreach($tempTags as $tempTag) {
        if(!array_key_exists($tempTag, $allTags)) {
            ...
            if ($result === false) Response::error('Could not insert tag "' . $tempTag . '"!');
            
            $insertId = Database::insertId($connection, 'update_030200', __LINE__);
            if ($insertId === false) Response::error('No insert ID for tag "' . $tempTag . '"!');
	...
	$result = Database::execute($connection, $query, 'update_030200', __LINE__);
        ...
        if ($result === false) Response::error('Could not insert photos to tags for photo:' . $photoWithTags->id. ' and tag: "' . $tempTag . '"!');
    }
}

// Drop tags column in Photos table
$query  = Database::prepare($connection, "ALTER TABLE `?` DROP COLUMN tags", array(LYCHEE_TABLE_SETTINGS));
$result = Database::execute($connection, $query, 'update_030200', __LINE__);

if ($result===false) Response::error('Could not drop tags column!');
